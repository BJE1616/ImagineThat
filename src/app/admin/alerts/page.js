...
export default function AlertsPage() {
    ...
    const getTypeLabel = (type) => {
        const labels = {
            prize_pick_winner: 'Winner Selection',
            prize_notify_winner: 'Winner Notification',
            prize_pay_winner: 'Winner Payment',
            prize_setup_missing: 'Prize Setup',
            health_critical: 'System Health',
            health_warning: 'System Health',
            payout_pending: 'Payouts',
        }
    ...
    return (
        <div className="p-4 max-w-4xl mx-auto">
            ...
            <div className="mb-6">
                ...
                <p className={`text-${currentTheme.textMuted} text-sm mt-1`}>
                    Action items requiring admin attention
                </p>
            </div>
            ...
            {/* Filters */}
            <div className={`flex items-center gap-2 mb-4 pb-4 border-b border-${currentTheme.border}`}>
                ...
                {[
                    ...
                    <button
                        ...
                        className={`px-3 py-1 rounded text-xs font-medium transition-all ${filter === f.key
                                ? `bg-${currentTheme.accent} text-slate-900`
                                : `bg-${currentTheme.card} text-${currentTheme.textMuted} hover:text-${currentTheme.text} border border-${currentTheme.border}`
                            }`}
                    >
                        {f.label}
                    </button>
                ...
                <button
                    onClick={fetchAlerts}
                    className={`ml-auto px-3 py-1 rounded text-xs font-medium bg-${currentTheme.card} text-${currentTheme.textMuted} hover:text-${currentTheme.text} border border-${currentTheme.border}`}
                >
                    ↻ Refresh
                </button>
            </div>
            ...
            {filteredAlerts.length === 0 ? (
                <div className={`text-center py-12 bg-${currentTheme.card} rounded-lg border border-${currentTheme.border}`}>
                    ...
                    <h3 className={`text-${currentTheme.text} font-medium mb-1`}>
                ...
                <div className="space-y-3">
                    {filteredAlerts.map((alert) => {
                        ...
                        return (
                            <div
                                key={alert.key}
                                className={`${styles.bg} border ${styles.border} rounded-lg p-4 transition-all hover:scale-[1.01]`}
                            >
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <h3 className={`text-${currentTheme.text} font-semibold`}>
                                            {alert.title}
                                        </h3>
                                        <p className={`text-${currentTheme.textMuted} text-sm mt-0.5`}>
                                            {alert.description}
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <button
                                            ...
                                            className={`px-3 py-1.5 ${styles.badge} text-white rounded text-sm font-medium hover:opacity-90 transition-all`}
                                        >
                                            {alert.actionLabel}
                                        </button>
                                        <button
                                            ...
                                            className={`px-3 py-1.5 bg-${currentTheme.card} text-${currentTheme.textMuted} hover:text-${currentTheme.text} border border-${currentTheme.border} rounded text-sm font-medium transition-all disabled:opacity-50`}
                                        >
                                            {dismissing === alert.key ? '...' : 'Dismiss'}
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            <div className={`mt-6 p-4 bg-${currentTheme.card} border border-${currentTheme.border} rounded-lg`}>
                ...
                <ul className={`text-${currentTheme.textMuted} text-xs space-y-1`}>
                    ...
                    <li>• Dismissing an alert hides it, but it will reappear if conditions still apply</li>
